...
/**
 * Represents a Thycotic DevOps Secrets Vault.
*/
export class Vault {
    ...
    */
    public async getSecret(path: string): Promise<Secret> {
        ...
        let response: string = await this.accessResource("GET", "secrets", path, null);
    ...
    */
    private async getResponseContent(response: httpClient.HttpClientResponse): Promise<string> {
        if (!this.isSuccessStatusCode(response.message.statusCode)) {
            console.log(`vault.getResponseContent: failed code ${response.message.statusCode}`);
            return "";
        }
